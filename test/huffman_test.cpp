...
void huffman_test::test_NodeComp() {
    TreeNode a(12, uint32_t('a'));
    TreeNode b(32, uint32_t('b'));
    NodeComp c;
    DO_CHECK(!c(&a, &b));
}
...
void huffman_test::test_HuffTree_built() {
    ...
    for (uint32_t i = 1; i < 13; i += 4)
        tree.add(new TreeNode(i, uint32_t('a' + i)));
    tree.build();
...
void huffman_test::test_calculate_codes() {
    ...
    bool exp1 = codes.at('H') == std::vector<bool>({1,0,1});
    bool exp2 = codes.at('o') == std::vector<bool>({1,1,1});
    bool exp3 = codes.at('\n') == std::vector<bool>({0,0});
    bool exp4 = codes.at('e') == std::vector<bool>({1,1,0});
    bool exp5 = codes.at('l') == std::vector<bool>({0,1});
    bool exp6 = codes.at('!') == std::vector<bool>({1,0,0});
